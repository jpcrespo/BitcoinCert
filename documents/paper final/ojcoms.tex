...
\currentdate{15 February, 2026}
\doiinfo{OJCOMS.2026.XXXXXX}

\title{Can Taproot Enable Scalable and Revocable Document Verification on Bitcoin Blockchain?
}

\author{
Juan Pablo Crespo Vargas\IEEEauthorrefmark{1},
Sultan Ahmad Al-Muhammadi\IEEEauthorrefmark{1},
Moatsum Alawida\IEEEauthorrefmark{2},
Farag Ahmed Azzedin\IEEEauthorrefmark{1}
}
\affil{\IEEEauthorrefmark{1}Information and Computer Science Department, King Fahd University of Petroleum and Minerals, Dhahran, Saudi Arabia}
\affil{\IEEEauthorrefmark{2}Department of Computer Science, Abu Dhabi University, Abu Dhabi 59911, United Arab Emirates}
\corresp{Corresponding author: Juan Pablo Crespo Vargas (e-mail: g202393610@kfupm.edu.sa).}


\markboth{Can Taproot Enable Scalable and Revocable Document Verification on Bitcoin Blockchain?}{Crespo Vargas \textit{et al.}}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{abstract}
Verifying document authenticity, integrity, and revocation at scale commonly depends on centralized verification services with explicit trust and availability assumptions.
Although Bitcoin is widely used for cryptographic anchoring, existing approaches lack a unified construction that jointly provides scalable batch commitments, privacy-preserving per-document verification, and explicit revocation within Taproot.
This paper presents a Taproot-based commitment architecture that enables document verification through a single Pay-to-Taproot (P2TR) transaction.
Document hashes are aggregated in a Merkle tree committed inside Taproot, while issuer authenticity is enforced via Schnorr signatures bound to the same commitment.
The design separates immutable issuance commitments from dynamic revocation state using transaction chaining, enabling revocation updates without re-anchoring the full batch.
We formally analyze security under standard cryptographic assumptions and model on-chain cost and verification complexity as functions of batch size.
We show that Merkle proof size, proof generation, and verification scale logarithmically as $\mathcal{O}(\log N)$ with the number $N$ of documents in the batch, while anchoring cost remains constant.
The results demonstrate that Taproot enables scalable, revocable, and privacy-preserving document verification at the Bitcoin base layer without protocol modifications or reliance on expressive smart-contract execution.
\end{abstract}

\begin{IEEEkeywords}
Bitcoin, Taproot, Merkle Commitments, Schnorr Signatures, Decentralized Document Verification, On-chain Revocation, Cryptographic Protocols.
\end{IEEEkeywords}
...
\IEEEPARstart{D}{ocument} fraud, particularly involving academic credentials, represents a significant global challenge~\cite{Xie2020EthereumCertificate}. Verification of document authenticity, integrity, and validity is traditionally handled by the issuing organizations themselves, creating a structural dependency on centralized verification infrastructures~\cite{shende2024enhancing}. 
...
The proposed design leverages non-executed Taproot script paths to encode cryptographic commitments, combined with Merkleized Abstract Syntax Trees (MAST) and Schnorr signatures standardized in BIPs~\cite{bip340,bip341}.
...
\begin{enumerate}
    ...
    \item We propose a revocation mechanism based on transaction chaining that updates the revocation state without modifying or invalidating the original issuance commitment.
...
Table~\ref{tab:abbrev} summarizes the main abbreviations used throughout the paper. 

Table~\ref{tab:notation} summarizes the principal symbols and parameters adopted in the protocol description, security model, and complexity analysis.
...
\begin{table*}[!t]
\centering
\caption{Main symbols and parameters used in the paper.}
\label{tab:notation}
\small
\renewcommand{\arraystretch}{1.15}
\begin{tabular}{ll}
...
$H_{\mathrm{sig}}(\cdot)$ & Hash function used to derive the signed message \\
...
$T_k$ & $k$-th descendant transaction in the revocation chain \\
$B_k$ & Revocation bitmap published in transaction $T_k$ \\
$d$ & Depth of the Merkle tree, typically $\lceil \log_2 N \rceil$ \\
...
$a,b$ & Implementation-dependent constants in the verification model \\
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=\textwidth]{images/scheme_commit.pdf}
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{images/merkle_tree.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=1\columnwidth]{images/verification.pdf}
...
If the issuer follows the protocol honestly and $D_i$ has not been revoked, then the verification algorithm accepts $(D_i, \pi_i)$ with probability $1$.
...
The construction satisfies batch integrity if no probabilistic polynomial-time (PPT) adversary $\mathcal{A}$ can produce a pair $(D^*, \pi^*)$ such that:

\begin{itemize}
    \item $D^*$ was not included in the original batch committed in transaction $T_0$, and
    \item the verification algorithm accepts $(D^*, \pi^*)$,
\end{itemize}
...
where the constants $a$ and $b$ depend on the implementation environment.
...
where $C_{\mathrm{rev}}$ represents the constant overhead of the transaction template. 
For a fixed batch size $N$, subsequent revocation updates do not increase this value because each update republishes a bitmap of identical length.
...
\begin{tabular}{lcccc}
\hline
\textbf{Approach} & \textbf{Issuance Cost} & \textbf{Verification} & \textbf{Revocation} & \textbf{Authentication} \\
\hline
\texttt{OP\_RETURN} Anchoring & $\mathcal{O}(N)$ & $\mathcal{O}(1)$ & No & No \\
Merkle Anchoring (e.g., OpenTimestamps) & $\mathcal{O}(1)$ & $\mathcal{O}(\log N)$ & No & No \\
Proposed Protocol & $\mathcal{O}(1)$ & $\mathcal{O}(\log N)$ & Yes & Yes \\
\hline
\end{tabular}
\end{table*}
...
Future work may explore privacy-enhancing extensions such as zero-knowledge proofs capable of hiding Merkle path positions or issuer metadata. 
While such approaches could improve unlinkability properties, they would also introduce additional computational overhead and implementation complexity.
